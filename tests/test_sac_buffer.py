...
def _fill(buf, n, action_dim=2):
    img = _img()
    goal = np.array([100.0, 200.0], dtype=np.float32)
    motion = np.zeros(4, dtype=np.float32)
    action = np.zeros(action_dim, dtype=np.float32)
    for _ in range(n):
        buf.store_transition(img, goal, motion, action, float(np.random.rand()),
                             img, goal, motion, False)


def test_store_and_sample_shapes():
    buf = SACReplayBuffer(max_size=1000, action_dim=2, device='cpu')
    _fill(buf, 200)
...
def test_images_stored_as_uint8_sampled_as_float():
    buf = SACReplayBuffer(max_size=100, action_dim=2, device='cpu')
    img = torch.full((3, 96, 96), 128, dtype=torch.uint8)
    goal = np.zeros(2, dtype=np.float32)
    motion = np.zeros(4, dtype=np.float32)
    buf.store_transition(img, goal, motion, np.zeros(2), 0.0, img, goal, motion, False)
...
def test_buffer_wraps_around():
    buf = SACReplayBuffer(max_size=50, action_dim=2, device='cpu')
    _fill(buf, 80)
